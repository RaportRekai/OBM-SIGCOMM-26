...
def plot_all_combined(agg_private, agg_shared, outdir, dpi: int):
    ...
    for dataset in all_datasets:
        ...
        # ==========================================================
        # SHORT-FLOW P99 FCT ONLY
        # ==========================================================
        private_series = build_series(
            agg_private,
            dataset,
            xvals,
            "short",
        ...
        if not _any_finite(private_series) and not _any_finite(shared_series):
            continue

        # Normalize both private and shared to shared-buffer OBM
        ref_vals = shared_series.get(BASELINE_LABEL, [])

        private_norm = _normalize_series(
            private_series,
            ref_vals,
        )

        shared_norm = _normalize_series(
            shared_series,
            ref_vals,
        )
        ...
        combined_legend(
            ax,
            colors,
            loc=_legend_loc_for(dataset, "short", "p99"),
        )
...
def main():
    ...
    ap.add_argument(
        "--private-dir",
        default="obm-sim-dctcp-websearch-n-priority-private",
        help="Folder containing private-buffer stats files.",
    )

    ap.add_argument(
        "--shared-dir",
        default="obm-sim-dctcp-websearch-n-priority-shared",
