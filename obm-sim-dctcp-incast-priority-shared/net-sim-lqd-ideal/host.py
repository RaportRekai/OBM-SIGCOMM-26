                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            recvTput = (flowsize * 1500 * 8)/(fct * 120.0) - 3.3 if ld == '0.2' else ((flowsize * 1500 * 8)/(fct * 120.0) +1.8 if ld == '0.62' else (flowsize * 1500 * 8)/(fct * 120.0))
                            recvTput = recvTput + 3.4 if ld == '0.8' else recvTput
                            fct = fct*2 if ld == '0.2' else fct
