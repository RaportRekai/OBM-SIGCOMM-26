...
class Switch():
    ...
    def __init__(self, addr, num_tor_ports, num_agg_ports, hosts_per_rack):
        ...
        self.per_port_max_qsize = 5*PACKET_SIZE  # in terms of number of 1500B packets
        self.K = 4*PACKET_SIZE                   # threshold for ECN marking (in terms of number of packets)
                ...
                if self.queues[port][(start+i)%len(self.queues[port])].empty():
                    ...
                    for j in range(0,self.queues[port][(start+i)%len(self.queues[port])].qsize()+1):
                        if packet.invalid == 0:
                            ...
                            self.port_qsize[port] -= PACKET_SIZE
                            self.sent+=1
                            self.total_usage -= PACKET_SIZE
                            self.voq_port_qsize[port-1][(start+i)%len(self.queues[port])]-= PACKET_SIZE
                            flag_1 = 1
...
################################################################################ BIT MAPPER ########################################################################################
        if self.total_buffer_size > self.total_usage:
            ...
            if (self.port_qsize[outPort] < self.T[outPort-1] and packet.priority != 1) or (self.port_qsize[outPort] < self.T_h[outPort-1] and packet.priority == 1):
                ...
                self.port_qsize[outPort] += PACKET_SIZE
                self.voq_port_qsize[outPort-1][packet.priority-1]+= PACKET_SIZE
