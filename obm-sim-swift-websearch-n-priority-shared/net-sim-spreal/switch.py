...
class Switch():
    ...
    def __init__(self, addr, num_tor_ports, num_agg_ports, hosts_per_rack):
        ...
        self.block_port = [[0,0] for i in range(self.N)]
        #########################################################################################################
        self.total_usage = 0 
        self.final_add = [0 for i in range(self.N)]
        self.T = self.total_buffer_size
        self.sent = 0
        # self.alpha = 2 # OLD ALPHA logic from first block
        self.t = 0
        self.k = 0
        self.t_track = 0
    ...
    def runSwitch(self, currTimeslot):
        ...
        if self.k>0:
            for ind,i in enumerate(self.buffer):
                if i[0] != -1:
                    self.block_port[ind][0] = 1
                    self.block_port[ind][1] = self.k*30
    ...
    def block_timer(self):
        for i in range(self.ports):
            if self.block_port[i][1] == 0:
                self.block_port[i][0] = 0
            elif self.block_port[i][0] == 1:
                self.block_port[i][1] -=1
...
######################################################################## Additional ######################################################################################
    ...
    def priority_encoder(self,longest_ind,k):
        for inPort in range(self.priority_classes):
            ...
            if self.voq_port_qsize[longest_ind-1][self.priority_classes -1 - inPort]>=k:
                #print(f"got {k} locations")
                return self.priority_classes -1 - inPort
    ...
    def fetch(self):
        ...
        for h in range(self.k):
            if not target_queue.empty():  # Ensure the queue is not empty
                ...
                # if target_queue.queue[target_queue.qsize()-c-1].dstAddr == 'h6' and target_queue.queue[target_queue.qsize()-c-1].srcAddr == 'h12':
                #     print("dropping to make space for LQD")
                #     breakpoint() 
    ...
    def allct(self,mem):
        ...
        for ind,i in enumerate(self.buffer):
            if i[1] != -1:
                req+=1
        if req > space:
            # Note: In original code, packets here might be dropped if req > space,
            # but they haven't been assigned a switch_uid yet because they haven't
            # successfully entered the queue or been handled via handleRecvdPacket
            # fully in the LQD context. The main logic usually happens in handleRecvdPacket.
...
###############################################################################################################################################################
    ...
    def handleRecvdPacket(self, inPort, packet, arrivalTime):
        ...
        # Note: We use existing qsize/usage before incrementing for the current packet
...
################################################################################ BIT MAPPER ########################################################################################
        ...
        if self.total_buffer_size > self.total_usage and self.block_port[inPort-1][0] == 0:
        ...
        elif self.block_port[inPort-1][0] == 0:
            if outPort != (self.largest_index):
                self.buffer[inPort-1] = [packet,outPort]
                self.k +=1   
                #print("Initiated LQD")  
            else:
                if packet.dstAddr == 'h6' and packet.srcAddr == 'h12':
                    # --- DATA ACQUISITION: UPDATE LOG (TAIL DROP) ---
                    self.packet_history[unique_id][4] = 1
                    # ------------------------------------------------
                    print("dropping without LQD")
                    breakpoint()   
        # else:
            # print(self.block_port[inPort-1][1])
            # print("dropping packets priority 1 --blocked") 
