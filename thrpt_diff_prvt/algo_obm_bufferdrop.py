...
def fetch(k,longest_ind,index_of_longest_sublist):
    ...
    if voq_len[longest_ind[1]][index_of_longest_sublist]!=0:
        ...
        queue_length = len(longest_queue[index_of_longest_sublist])
        if voq_len[longest_ind[1]][index_of_longest_sublist] < k and voq_len[longest_ind[1]][index_of_longest_sublist]!=0:
            ...
            for j in range(queue_length):
                #breakpoint()
                if longest_queue[index_of_longest_sublist][-((j+1)-len(sen))][0] == -8:
                    continue
                sen.append(longest_queue[index_of_longest_sublist].pop(-((j+1)-len(sen))))
                if sen[-1][1] == 0:
                    packet_dropped+=1
                len_lst[longest_ind[1]][0] -= 1
                voq_len[longest_ind[1]][index_of_longest_sublist]  -= 1
        ...
        if k != 0:
            ...
            # If the sublist becomes empty, maintain an empty list
                ...
                if not longest_sublist:
                    ...
                    try:
                        remove = [sen[-1][1],longest_ind[1],index_of_longest_sublist]        
                    except:
                        breakpoint()
                return sen
        else:
...
total_packets = 0
index_tracker = [-1]*N
packet_admission_status = [False for _ in range(N)]
def decide_and_bitmap(d_port,cell_len,s_port,cell_count):
    ...
    for i,k in enumerate(d_port):
        ...
        if buffer[i][buffer_lag - lag -1][0] != -1 or stop[i] == 1:
            if buffer[i][buffer_lag - lag -1][1] == 0 and (MEMORY_PER_QUEUE - prvt_buff[buffer[i][buffer_lag - lag -1][0]][i])>=buffer[i][buffer_lag - lag -1][3]:
                final_add[0][i] = -8
                prvt_buff[buffer[i][buffer_lag - lag -1][0]][i] += buffer[i][buffer_lag - lag -1][3]
                packet_admission_status[i] = True
            elif buffer[i][buffer_lag - lag -1][1] == 0 and (MEMORY_PER_QUEUE - prvt_buff[buffer[i][buffer_lag - lag -1][0]][i])<buffer[i][buffer_lag - lag -1][3]:
                packet_admission_status[i] = False

            
            if packet_admission_status[i] == True:
                final_add[0][i] = -8
    ...
    for i in range(N):
        ...
        if r!=[]:
            ...
            if (q_change == 1 and p_remove[0]!=0 and p_remove[2] == i):
                ...
                rm_cnt = 0
                free = []
                q_length = len(q[p_remove[1]][i])
                for j in range(q_length):
                    if rm_cnt == p_remove[0]:
                        break
                    if q[p_remove[1]][i][-(j+1 - len(free))][0] != -8:
                        free.append(q[p_remove[1]][i].pop(-(j+1 - len(free))))
                        if free[-1][1] == 0:
                            packet_dropped+=1
                        memory_manager.deallocate(free[-1][0])
                        free_blocks+=1
                        len_lst[p_remove[1]][0] -= 1
                        voq_len[p_remove[1]][i] -= 1
                        rm_cnt+=1
            ...
            if final_add[-1][i] == -8:
                ...
                len_lst[out][0] += 1
                voq_len[out][i] += 1
            elif buffer[i][-1][0] == p_remove[1] and p_remove[2] == i and q_change == 1 and p_remove[0]!=0:
                p_remove = [0,-1,-1]
                q_change = 0
                continue
...
def priority_encoder(longest_ind,k):
    for c in range(N):
        if voq_len[longest_ind[1]][c]-prvt_buff[longest_ind[1]][c] > k:
            index_of_longest_sublist = c
            return index_of_longest_sublist
    for c in range(N):
        if voq_len[longest_ind[1]][c]-prvt_buff[longest_ind[1]][c]  > 0:
            index_of_longest_sublist = c
            return index_of_longest_sublist
    return(0) 
...
######################################### analysing changed content ################################
            if reading[d] == count[d] - 1:
                ...
                # We are trying to prepare the NEXT packet/cell group.
                # That requires q[d][rr[d]][1], so the queue must have at least 2 entries.
                if len(cur_q) > 1:
                    next_packet_cells = cur_q[1][3]

                    if voq_len[d][rr[d]] >= next_packet_cells:
                        voq_len[d][rr[d]] -= next_packet_cells
                        p_y[d][rr[d]] = 1
                    else:
                        p_y[d][rr[d]] = 0
                else:
                    # No next element exists. Queue is draining/emptying.
                    p_y[d][rr[d]] = 0
...
def main(burst,incast,round):
    ...
    while True:
        t+=1
        log_q_sums(q,t,path = f"./q_log/q_sums_{burst}_{incast}_{round}_incast_set{sys.argv[4]}_obm.csv")
        # print(f"time = {t}")
        # if t>47000:
        #     print(buffer[9])
        #     print(q[7][9][-36:])
        ...
        if packet_served > checkpoint+1:
            checkpoint+=10
            #print(f"checkpoint reached for {packet_served} packets served")
