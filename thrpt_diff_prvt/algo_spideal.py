...
def voq_priority(voq_idx):
    """
    Lower return value means higher priority.

    Only VOQ 0 and VOQ 1 are high priority.
    VOQ 2 .. N-1 are low priority.
    """
    return 0 if voq_idx in (0, 1) else 0
...
gate = [[[0,0]for c in range(N)] for _ in range(N)]
...
# generate all (i,j) pairs, then pick the one whose q[i][j] has the greatest len()
        ...
        if d_port[i]!=-1:
            ...
            lqd_trig = 0
            if gate[d_port[i]][i][0] == 1 or cell_len[i] == 0:
                lqd_trig = 0
                #if there are free blocks then place packets
                if cell_len[i] == 0 and (MEMORY_PER_QUEUE - prvt_buff[d_port[i]][i]) >= cell_count[i]:
                ...
                elif cell_len[i] == 0 and (MEMORY_PER_QUEUE - prvt_buff[d_port[i]][i]) < cell_count[i] :
                    if free_blocks!=0:
                        ...
                        lqd_trig = 1
                elif cell_len[i] != 0 and gate[d_port[i]][i][1] == 1:
                    cell_add = -8
                    q[d_port[i]][i].append([cell_add,cell_len[i],s_port[i],cell_count[i]])
                    voq_len[d_port[i]][i] += 1
                elif cell_len[i] != 0 and gate[d_port[i]][i][0] == 1:
                    if free_blocks!=0:
                        cell_add = memory_manager.malloc()
                        if cell_add== None:
                            print(free_blocks)
                ...
                if lqd_trig == 1:
                    ...
                    eligible = [
                        ...
                        for v in range(N)
                        if len(q[d][v]) > 0
                        and voq_priority(v) > incoming_prio
                        and any(cell[0] != -8 for cell in q[d][v])
                    ]

                    if not eligible:
                        gate[d_port[i]][i] = [0, 0]

                        # This branch is reached only when shared memory is full.
                        # So if this packet has previous admitted cells, they should be shared cells.
                        if cell_len[i] != 0:
                            for c in range(cell_len[i]):
                                addr = q[d_port[i]][i][-(c + 1)][0]

                                if addr != -8:
                                    memory_manager.deallocate(addr)
                                    free_blocks += 1

                            del q[d_port[i]][i][-cell_len[i]:]
                    ...
                    # if they are not to the longest queue then just send them to their best place
                        ...
                        # Find the last / tail-side cell whose address is not -8

                        victim_idx = None

                        for idx in range(len(q[lq_o][lq_i]) - 1, -1, -1):

                            if q[lq_o][lq_i][idx][0] != -8:

                                victim_idx = idx

                                break

                        # If this longest queue has only -8 cells, do not evict from it
                        ...
                        victim_cell = q[lq_o][lq_i][victim_idx]

                        victim_seq = victim_cell[1]

                        # Find the head of the victim packet.

                        # Walk backward from victim_idx until seq == 0.

                        victim_head_idx = victim_idx

                        while victim_head_idx >= 0 and q[lq_o][lq_i][victim_head_idx][1] != 0:

                            victim_head_idx -= 1

                        # If no head found, something is structurally wrong

                        if victim_head_idx < 0:

                            print("ERROR: Could not find packet head for victim")

                            print(f"longest queue = q[{lq_o}][{lq_i}]")

                            print(f"victim_idx={victim_idx}, victim_cell={victim_cell}")

                            breakpoint()

                        # The packet portion to remove is from head through victim_idx.

                        # This removes the packet that owns the selected non--8 cell.

                        removed_packet = q[lq_o][lq_i][victim_head_idx:victim_idx + 1]

                        # Use the selected victim cell's real memory address for the incoming cell

                        if any(cell[0] == -8 for cell in removed_packet):

                            gate[d_port[i]][i] = [0, 0]

                            # Drop the incoming packet/cell because no valid shared-only victim exists.
                        ...
                        # Remove the old packet cells from the longest queue

                        del q[lq_o][lq_i][victim_head_idx:victim_idx + 1]

                        # Update voq_len for removed cells

                        voq_len[lq_o][lq_i] -= len(removed_packet)

                        # Free all removed cells EXCEPT the victim cell's address,

                        # because that address was reused for the incoming cell.

                        for cell in removed_packet:

                            if cell is victim_cell:

                                continue

                            if cell[0] == -8:

                                prvt_buff[lq_o][lq_i] -= 1
    ...
    for d in range(0,N):
        ...
        if (stall_s[d]<=track_s[d] and count[d]!=-1) or count[d] == 0:
            if reading[d]!=-1:
                ...
                #print(f"i am sending the packet {send} from {d}")
                bytes_sent[d] += send[2]
                if send[1] == 0:
                    packet_served+=1
                    #print(send)
...
def read_csv(burst,incast,round):
    ...
    csv_file_path = f"output_new_{burst}_{incast}_{round}_incast_val{sys.argv[4]}.csv"
    global n_dest_port
    global n_size
    n_dest_port = []
    n_size = []
...
def main(burst,incast,round):
    ...
    while True:
        t+=1
        log_q_sums(q,t,path = f"./q_log/q_sums_{burst}_{incast}_{round}_incast_val{sys.argv[4]}.csv")
        #print(f"time = {t}")
        #print(bytes_sent)
        # print(f"\nTime Slot {t}")
        # print(v[0])
