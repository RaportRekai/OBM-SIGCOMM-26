...
class Switch():
    ...
    def __init__(self, addr,load, num_tor_ports, num_agg_ports, hosts_per_rack):
        ...
        self.alpha = 4 if load =="0.9" else 8
        self.alpha = 4 if load == "0.3" else self.alpha
        self.t = 0
        self.track = 0
                ...
                for j in range(self.N):
                    ...
                    if self.voq_port_qsize[port_key-1][v_idx] > self.T:
                        ...
                        if hasattr(pq, 'queue') and len(pq.queue) > 0:
                            ...
                            if head_packet.invalid == 0 and head_packet.prvt == 0:  # Only expel if packet is valid and not already prioritized
                                # 1. Drop Packet
