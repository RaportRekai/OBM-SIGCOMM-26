...
class Network:
    ...
    def parseswitches(self, switchParams):
        ...
        for addr in switchParams:
            switches[addr] = Switch(addr, self.num_tor_ports, self.num_agg_ports, self.hosts_per_rack,sys.argv[3],sys.argv[5])
        return switches
    ...
    def addLinks(self):
        ...
        for addr1, addr2 in self.links:
            ...
            if addr1 in self.hosts:
                ...
                self.hosts[addr1].packetLogFile = open(f"logs/"+addr1+"-recvd-packets.txt", "a")
            if addr2 in self.hosts:
                self.hosts[addr2].link = link
                self.hosts[addr2].packetLogFile = open(f"logs/"+addr2+"-recvd-packets.txt", "a")
...
def main():
    ...
    protected = set(glob.glob(os.path.join(f'logs/', 'recvd-flows-*.txt')))
    files = glob.glob(f'logs/*')
    for f in files: 
