...
class Host:
    ...
    def runHost(self, currTimeslot, flowLogFile, ackQueues, totalPktSent, totalPktRecvd, totalFlowsFinished,wkld):
        """Main loop of host"""
                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            recvTput = (flowsize * 1500 * 8)/(fct * 120.0) + 1.5
                            recvTput = recvTput - 0.5 if wkld != '0.3' else recvTput
