        ...
        if self.k>0:
            for ind,i in enumerate(self.buffer):
                if i[0] != -1:
                    ...
                    self.block_port[ind][1] = self.k*100 #100 was 0.3 and 0.9 for 0.6 it was 30
            #breakpoint()
                        ...
                        if port != -1:
                            if not target_queue.empty():
                                ...
                                try:
                                    ...
                                    print("error in finding a valid packet for fetch")
                                    breakpoint()
...
###############################################################################################################################################################
    ...
    def handleRecvdPacket(self, inPort, packet, arrivalTime):
        ...
        if self.per_port_buffer[outPort-1][packet.priority-1] == 0 and self.block_port[inPort-1][0] == 0:
...
################################################################################ BIT MAPPER ########################################################################################
        elif self.total_buffer_size > self.total_usage and self.buffer[inPort-1][1] == -1 and self.block_port[inPort-1][0] == 0:
        ...
        elif self.buffer[inPort-1][1] != -1 and self.total_buffer_size > self.total_usage and self.block_port[inPort-1][0] == 0:
            self.total_usage +=1
        ...
        elif self.buffer[inPort-1][1] == -1 and self.block_port[inPort-1][0] == 0:
        ...
        #     breakpoint()
            ...
            #print("dropping packet port block")
            
            if packet.priority == 1:  
                print("Dropping packet with priority 1 --blocked")
