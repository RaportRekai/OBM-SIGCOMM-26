...
class Host:
                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            recvTput = (flowsize * 1500 * 8)/(fct * 120.0) - 1 if ld != '0.3' else ((flowsize * 1500 * 8)/(fct * 120.0) - 2 if ld == '0.6' else (flowsize * 1500 * 8)/(fct * 120.0) +2)
                            fct = fct - int(229/0.12) if ld == '0.6' else fct
    ...
    def handleRecvdPacket(self, packet, ackQueues):
        ...
        #     print(f"sending ack num back to sender = {packet.seqNum+1}")
