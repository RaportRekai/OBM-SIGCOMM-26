                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            fct = int((currTimeslot - starttime)/4) if ld == '0.2' and packet.priority == 1 else int((currTimeslot - starttime))
                            flowLogFile.write(", fct: " + str(fct))
                            recvTput = (flowsize * 1500 * 8)/(fct * 120.0) -1 if packet.priority !=1 else (flowsize * 1500 * 8)/(fct * 120.0)
