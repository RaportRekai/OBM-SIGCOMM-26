...
class Switch():
    ...
    def __init__(self, addr, load, num_tor_ports, num_agg_ports, hosts_per_rack,ld):
        """Initialize parameters"""
        self.addr = addr  # address of switch
        self.links = {}   # links indexed by port
        self.queues = {}  # virtual output queues per port
        self.per_port_max_qsize = 4 if ld == "0.2" else 3  # Max size in Bytes
                ...
                for port_key in sorted_ports:
                    ...
                    if self.voq_port_qsize[port_key-1][prio_idx] > self.T[prio_idx]:
                        ...
                        if hasattr(pq, 'queue') and len(pq.queue) > 0:
                            ...
                            if head_packet.invalid == 0 and head_packet.prvt == 0:  # Only expel if packet is valid and not already private
                ...
                if not self.queues[port][prio_idx].empty():
                    ...
                    while not self.queues[port][prio_idx].empty():
                        ...
                        if packet.invalid == 1:
                            ...
                            #packet.hops +=1
                            if packet.prvt == 1:
                                if self.per_port_buffer[port-1][prio_idx]==1:
                                    self.per_port_buffer[port-1][prio_idx] = 0
                                else:
                                    breakpoint()
                            else:
    ...
    def handleRecvdPacket(self, packet, arrivalTime):
        ...
        # ---------------------------------------------------------
        if self.per_port_buffer[outPort-1][prio_idx] == 0:
            self.per_port_buffer[outPort-1][prio_idx] = 1
