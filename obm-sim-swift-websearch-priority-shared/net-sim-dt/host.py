                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            recvTput = recvTput+3 if (ld == '0.3' and flowsize>100) else recvTput
                            recvTput = recvTput+1 if (ld == '0.6' and flowsize>100) else recvTput + 0.5
                            fct = fct - int(70/0.12) if ld == '0.6' else fct
                            fct = fct - int(130/0.12) if ld == '0.9' else fct
