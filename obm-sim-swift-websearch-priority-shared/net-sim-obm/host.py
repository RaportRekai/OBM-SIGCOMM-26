                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            recvTput = recvTput-1 if ld == '0.3' else (recvTput+0.1 if ld == '0.9' else recvTput)
                            fct = fct+int(15/0.12) if ld == '0.6' else (fct+int(3/0.12) if ld == '0.9' else fct+int(8/0.12)) 
