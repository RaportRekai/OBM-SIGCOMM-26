...
CSV_PATH = f"logs/per_j_sum_DT.csv"
os.makedirs(os.path.dirname(CSV_PATH), exist_ok=True)
with open(CSV_PATH, "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["t"] + [f"j{j}" for j in range(N)])
...
def plot_and_save_active_queues( i,
    ...
    """
    global active_ports
    BASE_DIR = Path("/home/dan/LQD/LQD/master/dt_logs") 
    # ----- build file paths --------------------------------------------------
...
def active_calc(alpha):
    ...
    #     if npc[f] == 0:
    #         npc[f] = 1
    #print(f"npc = {npc*alpha}")
    #print(f"free_blocks = {free_blocks}")
    for n1,i in enumerate(q):
        tresh[n1] = alpha*free_blocks
    # print(tresh)
    # print(free_blocks)
    # if q[0] != [[] for _ in range(N)]:
    #     print([sum(len(q[j][h]) for h in range(N)) for j in range(N)])
    # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
...
def LQD(d_port,cell_len,t,s_port,cell_count,alpha):
    ...
    row = [sum(len(q[j][h]) for h in range(N)) for j in range(N)]
    with open(CSV_PATH, "a", newline="") as f:
        csv.writer(f).writerow([t] + row)
    active_calc(alpha)           
    #print(np)
...
def main(burst,incast,round,alpha):
    ...
    while True:
        ...
        ray_of_death = 0
        for i in range(0,N):
            if track[i] > stall[i] and v[i]<len(size[i])-1:
                track[i] = 1
                d_port[i] = dest_port[i][v[i]]
                if dest_port[i][v[i]] != -1:
                    ray_of_death = 1
                if cell_len[i] == -1:
                    size[i][v[i]] = pad_to_cell(size[i][v[i]], cell_size)
                    s_port[i] = min(size_e[i][v[i]],cell_size)
                    cell_count[i] = math.ceil(size[i][v[i]]/cell_size)
                    # print("#################33")
                    # dfd
                else:
                    s_port[i] = min(size_e[i][v[i]],cell_size)
                    cell_count[i] = 0
                stall[i] = min(cell_size*8/(link_speed*time_period),size[i][v[i]]*8/(link_speed*time_period))
                size[i][v[i]] = size[i][v[i]]-cell_size
                size_e[i][v[i]] = size_e[i][v[i]]-cell_size
                cell_len[i] +=1
            elif dest_port[i][v[i]]!=-1 and len(dest_port[i]) != v[i]+1:
                ray_of_death = 1
                track[i]+=1
                d_port[i]=-1
                s_port[i]=0
