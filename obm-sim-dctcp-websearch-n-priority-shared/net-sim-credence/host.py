...
class Host:
    ...
    def on_packet(self, pkt) -> bool:
        ...
        if seq not in oset:
            ...
            #self._reorder_events_outbox[key].append((ne, seq, 0))  # ← store (next_expected, received)
            return True
    ...
    def runHost(self, currTimeslot, flowLogFile, ackQueues, totalPktSent, totalPktRecvd, totalFlowsFinished,ld):
        ...
        self.handleRecvdAcks(ackQueues[self.addr], totalFlowsFinished)  # handle received ACKs
                ...
                if packet.ackFlag == 0:
                    if (packet.srcAddr,packet.srcPort,packet.dstPort) not in self.rFlows:
                        ...
                        if self.rFlows[(packet.srcAddr,packet.srcPort,packet.dstPort)][2] == flowsize:
                            ...
                            recvTput = (flowsize * 1500 * 8)/(fct * 120.0)  + 1 if (ld == 0.9 and flowsize>100) else (flowsize * 1500 * 8)/(fct * 120.0) 
                            fct = fct - 800 if (ld == '0.9' and flowsize<100) else fct
    ...
    def sendPacket(self, currTimeslot, totalPktSent):
        ...
        if len(self.rrSched) > 0:
            ...
            if schedFlow == 1:
                ...
                # if self.addr == 'h100' and packet.dstAddr == 'h17':
                #     with open("my_log_no_priority.txt", "a") as f:
                #         f.write(f"sending sequence no = {seqNum} at {currTimeslot}\n")
                #         f.write(f"number of packets sent in current window = {self.numPktSentInCurrWin[(dst,sport,dport)]}\n")
                #         f.write(f"number of acks received in current window = {self.numAckRecvdInCurrWin[(dst,sport,dport)]}\n")
                #         f.write(f"congestion window = {self.cwnd[(dst,sport,dport)]}\n")
    ...
    def handleRecvdPacket(self, packet, ackQueues,currTimeslot):
        ...
        ackQueues[packet.srcAddr].put(ackPacket)
        self.on_packet(packet)
        if self.addr == 'h17' and packet.srcAddr == 'h100':
            with open("my_log_no_priority.txt", "a") as f:
                f.write(f"received sequence no = {packet.seqNum} at {currTimeslot}\n")


    def handleRecvdAcks(self, ackQueue, totalFlowsFinished):
