...
# Occamy Globals
occamy_global_rr = 0          # Pointer for the Global Egress Port RR
occamy_local_rr = [0] * N     # Pointers for each Egress Port's internal RR (input selection)
occamy_is_evicting = False    # State machine: Are we busy deleting a packet?
occamy_victim_coords = (-1, -1) # (egress, ingress) of current victim
occamy_cells_to_remove = 0    # Counter for the current eviction cycle
serving_valid_packet = [True] * N
...
# Initialize this with your other globals
occamy_clk_tick = 0 

def occamy_manager(alpha):
    global occamy_global_rr, occamy_local_rr
    global occamy_is_evicting, occamy_victim_coords, occamy_cells_to_remove
    global free_blocks, voq_len, q, packet_dropped
    global occamy_clk_tick # <--- New Global
    
    # ============================================================
    # 1. CLOCK DIVIDER (Slow down eviction)
    # ============================================================
    # Increment counter every cycle
    occamy_clk_tick += 1
    
    # If this is an EVEN cycle, do nothing. Return immediately.
    # This ensures we only perform an action (search or evict) 
    # once every 2 cycles.
    if occamy_clk_tick % 2 == 0:
        return
    # ============================================================

    # 2. State Machine: Continue eviction if already started
    if occamy_is_evicting:
        o, i = occamy_victim_coords
        
        # Safety: If queue became empty by scheduler, stop evicting
        if not q[o][i] :
            occamy_is_evicting = False
            return
        if q[o][i][-1][0]==-8:
            occamy_is_evicting = False
            return
        ...
        # --- EVICTION ACTION ---
        # Pop the TAIL cell (Now effectively happens every 2 cycles)
        victim_cell = q[o][i].pop() 
        memory_manager.deallocate(victim_cell[0])
        free_blocks += 1
        voq_len[o][i] -= 1
    ...
    # 3. Logic: Find new victim (if not evicting)
    # 3. Logic: Find new victim (if not evicting)
    limit = alpha * free_blocks 
    
    # rich_ports = []  <--- DELETE THIS
    
    for offset in range(N):
        o = (occamy_global_rr + offset) % N
        total_egress_cells = sum(len(q[o][x]) for x in range(N))
        
        if total_egress_cells > limit:
            # rich_ports.append(o) <--- DELETE THIS
            
            victim_o = o
            # ... rest of logic ...
            found_voq = False
            
            start_local = occamy_local_rr[victim_o]
            for local_offset in range(N):
                input_idx = (start_local + local_offset) % N
                if len(q[victim_o][input_idx]) > 0 and q[victim_o][input_idx][-1][0] != -8:
                    occamy_victim_coords = (victim_o, input_idx)
                    occamy_is_evicting = True
                    found_voq = True
                    
                    occamy_local_rr[victim_o] = (input_idx + 1) % N
                    occamy_global_rr = (o + 1) % N
                    break
            
            if found_voq:
                # Perform the FIRST eviction immediately
                # (This will also be subject to the 2-cycle delay next time)
                o_v, i_v = occamy_victim_coords
                cell = q[o_v][i_v].pop()
                memory_manager.deallocate(cell[0])
                free_blocks += 1
                voq_len[o_v][i_v] -= 1
                
                seq = cell[1]
                if seq > 0 and len(q[o_v][i_v]) >= seq:
                    q[o_v][i_v][-seq][4] = 0
...
def allct(r,t):
    ...
    for i, k in enumerate(final_add[-1]):
        ...
        # If memory was allocated in bit_mapper
        if final_add[-1][i] != -1:
            
            dest_port = buffer[-1][i][0]
            seq_num = buffer[-1][i][1]
            
            # --- CONTINUITY CHECK START ---
            is_valid_sequence = False
            
            # Case 1: Start of a new packet (Seq 0)
            if seq_num == 0:
                is_valid_sequence = True
                
            # Case 2: Continuation of a packet (Seq > 0)
            else:
                # We can only accept Seq N if the queue currently ends with Seq N-1
                if len(q[dest_port][i]) > 0 and q[dest_port][i][-1][1] == (seq_num - 1):
                    is_valid_sequence = True
                else:
                    is_valid_sequence = False
            # --- CONTINUITY CHECK END ---

            if is_valid_sequence:
                # [Action: Admit]
                # Added , 1 at the end for 'valid_bit'
                q[dest_port][i].append([final_add[-1][i], buffer[-1][i][1], buffer[-1][i][2], buffer[-1][i][3], 1])
                voq_len[dest_port][i] += 1
                
                # Update tracker: [last_sequence, active_status]
                last_q[i] = [seq_num, 1]
            
            else:
                # [Action: Reject (Sanity Check Failed)]
                # This catches cases where Seq 0-5 were dropped, but Seq 6 tried to enter.
                
                # 1. Return the memory block immediately
                if final_add[-1][i] == -8:
                    prvt_buff[dest_port][i] -= 1
                else:
                ...
                # 3. Update tracker to show this input is effectively inactive/dropping
                last_q[i] = [seq_num, 0]
                
                # print(f"Safety Drop! Input {i} tried to insert Seq {seq_num} but Queue Tail mismatch.")

        # If memory was NOT allocated (dropped in bit_mapper)
        elif 0 == buffer[-1][i][1]:
            # Just record the drop for statistics
            packet_dropped += 1
            last_q[i] = [0, 0]

        # [Logic for handling 'no cells left' cases remains the same]
        if buffer[-1][i][0] != -1 and last_q[i][1] == 1 and final_add[-1][i] == -1:
            last_q[i][1] = 0
            if buffer[-1][i][1] == 0:
                packet_dropped += 1
            else:
                # Standard drop logic if run out of space mid-stream
                # (Though the continuity check above now catches the *next* cell)
                ...
                for c in free:
                    ...
                    free_blocks += 1
                breakpoint()
                 # Important: Since we just deleted the tail, any subsequent incoming 
                 # cells for this packet will fail the 'is_valid_sequence' check next cycle.
    ...
    for d in range(0,N):
        ...
        if (stall_s[d]<=track_s[d] and count[d]!=-1) or count[d] == 0:
            if reading[d] != -1:
                ...
                # 1. If this is a HEAD cell (Sequence 0), we determine the fate of the whole packet
                if send[1] == 0:
                    if send[4] == 1:
                        serving_valid_packet[d] = True  # Packet is good, serve this and subsequent cells
                    else:
                        serving_valid_packet[d] = False # Packet is marked invalid, drop this and subsequent cells

                # 2. Decide action based on the STATE, not just the individual cell
                if serving_valid_packet[d]:
                    # NORMAL BEHAVIOR: Credit the bytes and count packet
                    bytes_sent[d] += send[2]
                    if send[1] == 0:
                        packet_served += 1
                else:
                    # INVALID PACKET STATE: Just waste the cycle
                    # We popped the cell and freed memory, but we do NOT credit bytes_sent
                    # This simulates "processing" a dead packet without sending it
                    pass

                # --- LOGIC END ---

                # Always free the memory block
                if send[0] != -8:  # Only deallocate if this cell had an actual memory block (not the special "accepted but waiting" code)
                    memory_manager.deallocate(send[0])
                    free_blocks += 1
                else:
                    prvt_buff[d][rr[d]] -= 1 # Free the reserved private buffer space
...
######################################### analysing changed content ################################
    ...
    # We just finished the current packet.

    # Only inspect q[d][rr[d]][1] if it physically exists.

                if len(q[d][rr[d]]) > 1 and voq_len[d][rr[d]] > 1:

                    next_pkt_cells = q[d][rr[d]][1][3]

                    if voq_len[d][rr[d]] >= next_pkt_cells:

                        voq_len[d][rr[d]] -= next_pkt_cells
...
def main(burst,incast,round,alpha):
    ...
    while True:
        t+=1
        log_q_sums(q,t,path = f"./q_log/q_sums_{burst}_{incast}_{round}_{alpha}_occamy.csv")
        # print(f"\nTime Slot {t}")
        # #print(bytes_sent)
        # print(f"free blocks = {free_blocks}")
        # for c in range(N):
