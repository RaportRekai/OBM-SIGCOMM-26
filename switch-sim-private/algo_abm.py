...
packet_admission_status = [[False,0] for _ in range(N)]  # Persists decision for duration of packet
#################################################################cleared################################################
def bit_mapper(d_port,cell_len,s_port,cell_count):
    ...
    for i in range(N):
        if d_port[i] != -1:
            buffer[0][i] = [d_port[i],cell_len[i],s_port[i],cell_count[i],0] # last is for private buffer or not
            if cell_len[i] == 0:
                total_packets+=1
