...
TOTAL_MEMORY = 26214 #+ 304 #int(22 * 1024 * 1024/cell_size)
cell_size = 80
            ...
            if gate[d_port[i]][i] == 1 or cell_len[i] == 0:
                ...
                # if u do not have free blocks, then we have to remove some packets from the longest queue
                    ...
                    if not eligible:
                        ...
                        # If this is a later cell of a packet, remove already admitted cells
                        # from the same packet.
                        if cell_len[i] != 0:
                            for c in range(cell_len[i]):
                                memory_manager.deallocate(q[d_port[i]][i][-(c + 1)][0])
...
def main(burst,incast,round):
    ...
    while True:
        t+=1
        log_q_sums(q,t,path = f"./q_log/q_sums_{burst}_{incast}_{round}_{sys.argv[4]}lqd.csv")
        #print(f"time = {t}")
        #print(bytes_sent)
        # print(f"\nTime Slot {t}")
        # print(v[0])
